...
"""
    try:
        with st.spinner("Generating prompt..."):
            ...
            # response.text is typical; provide fallbacks for different response shapes
            generated = None
            if hasattr(response, "text"):
                generated = response.text
            elif hasattr(response, "candidates") and len(response.candidates) > 0:
                # some SDK versions put content in candidates[0].content
                candidate = response.candidates[0]
                generated = getattr(candidate, "content", None) or getattr(candidate, "text", None)
